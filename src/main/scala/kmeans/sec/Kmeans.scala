...
import kmeans.utils.{CPoint, euclidean,error}
...
class Kmeans(K:Int,iters:Int,points:List[CPoint]){
  ...
  def train:List[Cluster] = {
    ...
    def update(oldClusters:List[Cluster], pts:List[CPoint],iter:Int):List[Cluster] = {
      ...
      val tmpcluster = old_clusters.map(_.moveCenter) // Para cada cluster generamos uno nuevo cambiando el centroide
  ...
  def init(points:List[CPoint]):List[Cluster] = {
    ...
    val centroids = (0 until K).map({
      u => points(rand.nextInt(points.size))
    })
  ...
  private def assignClusters(clusters: List[Cluster], points: List[CPoint]):List[Cluster] = {
    ...
    points
      ...
      .map({
        case (id_cluster,pts) =>
          Cluster(clusters(id_cluster).center,pts) // Para cada grupo creamos un cluster con la lista de puntos correspondiente
      })
      .toList
  }
...
object Kmeans {
  ...
  /**
    * Crea un objeto [[Kmeans]] dado un valor K de clusters,
    * un valor iters máximo de iteraciones y una lista de puntos points.
    * @param K Numero de clusters
  ...
  private def computeB(cls:List[Cluster]):List[List[Double]] = {
    ...
    cls
      .zipWithIndex
      .map( x=> x._1.members
        .map(u => cls.zipWithIndex.filter(_._2!=x._2)
          .map(y => y._1.members
            .map(euclidean(_,u))
  ...
  def silhouette(cls:List[Cluster]):Double = {
    ...
    val a=computeA(cls)
    val b=computeB(cls)
    val npoints = cls.map(_.members.size).sum

    (a.flatten,b.flatten).zipped.map{
  ...
  def distortion(cls:List[Cluster]):Double = {
    ...
    cls.map(u => u.members.map(error(_,u.center)).sum).sum
  }
